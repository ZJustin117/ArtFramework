...
| `room-shells` | 10 | `AbstractRoom`, `CampfireUI`, `TreasureRoom`, `NeowRoom` | Delegation-governed (hooked 2/10: rest, treasure); hooked members remain `ART_DELEGATED` with unchanged policy. Remaining room-shell paths use observe-first `RoomShellView` / `Sts1RoomShellProjection` metadata and a resource-backed chrome/label overlay only; native room pixels continue and no concrete room patch or suppression is added. |
...
| `cards-piles-soul` | 7 | `AbstractCard`, `CardGroup`, `Soul`, `SoulGroup` | Mixed observe-first overlay family: `AbstractCard#render` is `OUT_OF_SCOPE` by SDD boundary (ART never patches or suppresses card pixels); `CardGroup`, draw/discard/exhaust/deck piles, and `Soul`/`SoulGroup` publish dependency-neutral pile/soul chrome (`id`, `zone`, `count`, `bounds`, `visible`, `resourceId`, `label`) through `PileSoulView` / `Sts1PileSoulProjection`. `Sts1PileSoulDrawPath` may draw resource-backed labels/icons with unknown-resource fallback, but native card-group/soul render paths and live card pixels always continue. |
