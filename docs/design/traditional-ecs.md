...
| 4 | `PresentSurfaces` mounted flag and entity-ID map | `SurfaceLifecycleComponent` and stable surface key | Mount/unmount/reset/probe JUnit; lifecycle facade dispatches a synchronous transient runtime signal and the durable mounted state remains ECS-owned |
...
| 9 | Native intent execution and pending gestures | Intent lifecycle/result components; executor as host boundary only | Complete: router/executor outcomes cross the host boundary as synchronous transient runtime signals consumed by `NativeIntentLifecycleSystem`; durable `NativeIntentLifecycleComponent` and `NativeIntentObservationComponent` remain ECS-owned, with the next available authority frame transitioning executed intent to `CONFIRMED` and an unavailable frame transitioning it to `FAILED`. Domain-specific business confirmation remains separate |
