...
public class CombatIntentRenderPatchesTest {
    ...
    @Test
    public void fullReadySuppressesNativeIntentRender() {
        mountedCombat();
        FullPresentMode.setIntentsLevel(PresentLevel.FULL);
        CombatInputRouter.setExecutor(new RecordingIntentExecutor());

        SpireReturn<Void> result =
                CombatIntentRenderPatches.ObserveNativeIntent.Prefix(null, null);

        assertTrue("FULL + mounted + combat + ready executor must suppress AbstractMonster.renderIntent",
                result.isPresent());
    }

    @Test
    public void offContinuesNativeIntentRender() {
        mountedCombat();

        SpireReturn<Void> result =
                CombatIntentRenderPatches.ObserveNativeIntent.Prefix(null, null);

        assertFalse("OFF must let AbstractMonster.renderIntent continue", result.isPresent());
    }

    @Test
    public void fullWithoutExecutorContinuesNativeIntentRender() {
        mountedCombat();
        FullPresentMode.setIntentsLevel(PresentLevel.FULL);
