...
public class EntityPresentTest {
    ...
    public void attachSyncLayoutDetach() {
        EntityPresent p = SpireUI.entities();
    ...
    public void reattachReplacesSlotAndNotifies() {
        DefaultEntityPresent p = SpireUI.entityPresent();
    ...
    public void unknownKindThrows() {
        SpireUI.entities().attach("x", "weapon", "x");
    }

    @Test(expected = IllegalArgumentException.class)
    public void syncRequiresAttach() {
        SpireUI.entities().sync("missing", null);
    }

    @Test(expected = IllegalArgumentException.class)
    public void layoutRequiresAttach() {
        SpireUI.entities().layout("missing", 0f, 0f, 1f);
    }
    ...
    public void resetForTestsClearsEntities() {
        SpireUI.entities().attach("z", "player", "z");
        SpireUI.resetForTests();
        assertEquals(0, SpireUI.entities().size());
    }

    @Test
    public void facadeSameInstanceAsRuntime() {
        assertSame(NativeTemplateRuntime.entities(), SpireUI.entities());
    }
