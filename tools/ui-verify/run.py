#!/usr/bin/env python3
"""CLI: run SpireUI UI-verify YAML scenarios."""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="SpireUI UI-layer YAML verifier")
    p.add_argument("scenarios", nargs="+", type=Path, help="YAML scenario paths")
    p.add_argument(
        "--device",
        action="store_true",
        help="Force device mode (requires SPIREUI_D1_SERIAL; probe console later)",
    )
    p.add_argument(
        "--out-dir",
        type=Path,
        default=None,
        help="Write result JSON (default: debug-artifacts/ui-verify or SPIREUI_UI_VERIFY_OUT_DIR)",
    )
